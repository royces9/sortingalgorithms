...
void swim_heap(char *array, int child, int size_e, int (*compare)(void *, void *)) {
	...
	while(child && (compare(array + child * size_e, array + parent * size_e) > 0)) {
...
void sink_heap(char *array, int size_a, int size_e, int (*compare)(void *, void *)) {
	int child = compare(array + 2 * size_e, array + size_e) > 0 ? 2 : 1;
	int parent = 0;

	while((child < size_a) && (compare(array + child * size_e, array + parent * size_e) > 0)) {
		swap(array + child * size_e, array + parent * size_e, size_e);

		parent = child;
		child = 2 * (parent + 1);

		if((child < size_a) && (compare(array + (child - 1) * size_e, array + child * size_e) > 0))
			--child;
	}
}
